...
const workflowIconsDir = resolve(projectRoot, 'node_modules/@spectrum-web-components/icons-workflow/icons');
const ICONS_CATALOG = process.env.ICONS_CATALOG === '1';
const BANNER = '/* eslint-disable */\n/* Generated by da-express-milo Spectrum Web Components bundler */\n';

async function getWorkflowIconModules() {
  const files = await readdir(workflowIconsDir);
  return files
    .filter((file) => file.endsWith('.js'))
    .sort();
}

function getTagNameFromModuleFile(fileName) {
...
if (ICONS_CATALOG) {
  const allWorkflowIconModules = await getWorkflowIconModules();
  const allWorkflowIconTags = allWorkflowIconModules.map(getTagNameFromModuleFile);

  newComponents.push({
    name: 'icons-workflow-catalog',
    entry: allWorkflowIconModules
      .map((moduleFile) => `import '@spectrum-web-components/icons-workflow/icons/${moduleFile}';`)
      .join('\n'),
    // In catalog mode we must bundle every icon module, not externalize
    // to the curated icons-workflow dist bundle.
    skipExternals: ['./icons-workflow.js'],
  });

  await writeFile(
    resolve(distDir, 'icons-catalog.js'),
    `${BANNER}export const ICON_NAMES = ${JSON.stringify(allWorkflowIconTags, null, 2)};\n`,
    'utf8',
  );
}
...
/*  Build                                                              */
/* ------------------------------------------------------------------ */
console.log(`Building Spectrum Web Components bundles${ICONS_CATALOG ? ' (with full icons catalog)' : ' (new components only)'}...\n`);
