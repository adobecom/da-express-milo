...
| **Palette cards** | `.color-explorer-strips` only. Card → `.palette-card__strip-wrap` (strip) → footer. **Strip↔footer gap 10px** (`--palette-card-strip-footer-gap`). Card and footer `overflow: visible`; only strip-wrap clips so focus rings never clipped. L/M/S: 518/410/342, heights 80/56/56. Footer 32px; name Label-M; actions gap 6; action buttons 32×32, radius 8, focus ring 2px. |
