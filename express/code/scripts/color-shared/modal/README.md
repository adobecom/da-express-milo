...
This folder: `createModalManager.js`, `modal-styles.css`. Tokens: `../color-tokens.css`. Content is passed by the consumer. Shared utilities live in `../utils/` — screen reader announcements (`accessibility.js`), overlay primitives such as escape-close and focus trap (`overlay.js`), and Spectrum 2 icons (`icons.js`).

---

## Contract

- **Shell provides:** Curtain, container, close button (direct child of container), optional title (h2), content slot. Layout: mobile-first — base = drawer (bottom sheet); 600px tablet (M); 1200px desktop modal (L). Breakpoints 600 / 1200. All classes **ax-color-** prefix; body state `.ax-color-modal-open`. Close icon: Spectrum 2 `<sp-icon-close>` (via `createSpectrumIcon`). No header wrapper. All content is supplied by the consumer.
