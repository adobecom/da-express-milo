...
const lanaOptions = {
  sampleRate: 1,
  tags: 'express,Project Unity (Express), verb-express-hero',
  severity: 'error',
};
...
window.addEventListener('analyticsLoad', async ({ detail }) => {
  ...
  if (
    ...
    window.lana?.log(
      'Analytics failed to initialize correctly: some methods remain no-ops on verb-express-hero block',
      lanaOptions,
    );
  }
});
...
export default async function decorate(element) {
  ...
  // Converts the authored "Create your resume" link into a proper button
  // the milo way, before any of the DOM restructuring below. Requires the
  // CTA to be authored as a bold link (<strong><a>...</a></strong>) — milo's
  // decorateButtons only matches `em a, strong a, p > a strong`.
  const { decorateButtons } = await import(`${miloLibs}/utils/decorate.js`);
  decorateButtons(element, 'button-xl');

  // This block authors a text cell (heading/body/CTA link) and an image
  // cell per row, unlike verb-dropzone's single text cell.
  const [row] = element.querySelectorAll(':scope > div');
  const [textCell, imageCell] = row.children;
  const picture = imageCell?.querySelector('picture');

  const headline = textCell.querySelector('h1, h2, h3, h4, h5, h6');
  const paras = [...textCell.querySelectorAll('p')];
  const ctaPara = paras.find((p) => p.querySelector('a'));
  const bodyParas = paras.filter((p) => p !== ctaPara);
  ...
  // Marquee copy: logo + authored heading/body, kept separate from the
  // dropzone's own heading/subcopy.
  const copy = createTag('div', { class: 'copy' });
  if (headline) copy.append(headline);
  ...
  const ctaDropzone = createTag('div', { class: 'cta-dropzone' });
  if (ctaPara) ctaDropzone.append(ctaPara);
  ctaDropzone.append(dropzone);

  // Mirrors Figma's "cta-dropzone-legal" group, same reasoning as above.
  const actionGroup = createTag('div', { class: 'action-group' });
  actionGroup.append(ctaDropzone, footer);

  const copyColumn = createTag('div', { class: 'copy-column' });
  copyColumn.append(headingGroup, actionGroup);
  if (fileInput) copyColumn.append(fileInput);

  const imageColumn = createTag('div', { class: 'image-column' });
  ...
  element.textContent = '';
  element.append(foreground, errorState);
