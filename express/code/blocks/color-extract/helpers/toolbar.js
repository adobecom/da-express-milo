...
export default async function createToolbar(options = {}) {
  ...
  const toolbar = createTag('div', {
    ...
    role: 'toolbar',
    'aria-label': strings.toolbarAria,
  });
  ...
  if (options.onAddColor) {
    const addBtn = await createIconButton(
      ICON_EYEDROPPER,
      strings.toolbarAddColor,
      options.onAddColor,
    );
  ...
  const resetBtn = await createIconButton(ICON_REVERT, strings.toolbarReset, options.onReset);
  actionsGroup.append(resetBtn);
  focusableElements.push(resetBtn);

  const replaceBtn = await createIconButton(
    ICON_REPLACE,
    strings.toolbarReplaceImage,
    options.onReplace,
  );
  ...
  const undoBtn = await createIconButton(UNDO_ICON, strings.toolbarUndo, options.onUndo);
  const redoBtn = await createIconButton(REDO_ICON, strings.toolbarRedo, options.onRedo);
