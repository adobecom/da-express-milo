...
async function fetchBlogIndex(locales) {
  ...
  if (debug) console.info('blog-posts: query-index urls', urls);

  const resp = await Promise.all(urls.map((url) => fetch(url)
    .then(async (res) => {
      if (debug) console.info('blog-posts: query-index response', url, res.status);
      if (!res.ok) return null;
      const json = await res.json();
      if (debug) console.info('blog-posts: query-index items', url, json?.data?.length || 0);
      return json;
    })))
