...
export default async function decorate(block) {
  ...
  BlockMediator.set(stateKey, {
    selectedItem: null,
    currentData: initialData.slice(0, 24), // Load 24 initially
    allData: initialData, // Keep full dataset (34 total)
    searchQuery: '',
    filters: config.filters || [],
    // Pagination state
    currentPage: 1,
    pageSize: 24,
    hasMore: initialData.length > 24,
  ...
  const renderer = createColorRenderer(config.variant, {
    data: initialData.slice(0, 24), // Start with first 24 items
    config,
    dataService,
    modalManager,
    stateKey,
  ...
  renderer.on('filter-change', ({ type, value }) => {
    const currentState = BlockMediator.get(stateKey);
    ...
    // Apply filters
    const filteredData = applyFilters(currentState.allData, {
      filterType: newState.filterType,
      filterCategory: newState.filterCategory,
      filterTime: newState.filterTime,
    });

    // Paginate
    const paginatedData = paginateData(filteredData, 1, newState.pageSize);
    
    newState.currentData = paginatedData;
    newState.hasMore = filteredData.length > newState.pageSize;
    newState.totalCount = filteredData.length;

    BlockMediator.set(stateKey, newState);
    renderer.update(paginatedData);
  });

  // Listen for load more clicks
  renderer.on('load-more', () => {
    const currentState = BlockMediator.get(stateKey);
    ...
    const filteredData = applyFilters(currentState.allData, {
      ...
      filterTime: currentState.filterTime,
    });

    // Get next page of data
    const paginatedData = paginateData(filteredData, nextPage, currentState.pageSize);
    
    // Combine with existing data
    const combinedData = [...currentState.currentData, ...paginatedData];

    const newState = {
      ...currentState,
      currentPage: nextPage,
      currentData: combinedData,
      hasMore: combinedData.length < filteredData.length,
    };

    BlockMediator.set(stateKey, newState);
    renderer.update(combinedData);
  });

  // Listen for data updates and refresh
  dataService.on('data-fetched', (newData) => {
    // Update state
    const currentState = BlockMediator.get(stateKey);
    BlockMediator.set(stateKey, { ...currentState, allData: newData, currentData: newData });
...
function applyFilters(data, filters) {
  ...
  // Type filter (linear/radial)
  if (filters.filterType && filters.filterType !== 'all') {
    filtered = filtered.filter((item) => item.type === filters.filterType);
  }

  // Category filter (would need tags/categories in data)
  if (filters.filterCategory && filters.filterCategory !== 'all') {
    filtered = filtered.filter((item) => {
      const name = (item.name || '').toLowerCase();
      return name.includes(filters.filterCategory.toLowerCase());
    });
  }

  // Time filter (would need timestamp in data)
  // TODO: Implement when data has timestamps
