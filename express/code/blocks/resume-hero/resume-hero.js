...
const inlineIconRequests = new Map();

async function loadInlineIconSvg(iconUrl) {
  if (!inlineIconRequests.has(iconUrl)) {
    const request = fetch(iconUrl)
      .then(async (response) => {
        if (!response.ok) throw new Error(`Unable to load icon: ${iconUrl}`);
        return response.text();
      })
      .catch((error) => {
        inlineIconRequests.delete(iconUrl);
        throw error;
      });
    inlineIconRequests.set(iconUrl, request);
  }
  return inlineIconRequests.get(iconUrl);
}

function createInlineSvg(svgText, ownerDocument) {
  ...
  if (svg.nodeName.toLowerCase() !== 'svg' || svgDocument.querySelector('parsererror')) return null;

  const inlineSvg = ownerDocument.importNode(svg, true);
...
/**
 * Converts Milo-authored :icon-name: tokens to icon spans and injects their local SVGs.
 * Existing icon spans are decorated unless they have both data-svg-injected and an SVG.
 * @param {ParentNode} root Container holding authored icons.
 * @param {string} iconBasePath Directory containing the SVG icon files.
 * @param {Object<string, string>} iconAliases Authored icon names mapped to SVG filenames.
 */
export async function decorateMiloIcons(
  root = document,
  iconBasePath = '/express/code/icons',
  iconAliases = {},
) {
  const ownerDocument = root.ownerDocument || document;
  const NodeFilterRef = ownerDocument.defaultView?.NodeFilter || NodeFilter;
  const tokenPattern = /:([a-z0-9][a-z0-9_-]*):/gi;
  const walker = ownerDocument.createTreeWalker(root, NodeFilterRef.SHOW_TEXT);
  const textNodes = [];
  while (walker.nextNode()) {
    const { currentNode } = walker;
    const excluded = currentNode.parentElement?.closest('code, pre, script, span.icon, style');
    if (!excluded && tokenPattern.test(currentNode.textContent)) textNodes.push(currentNode);
  ...
  const basePath = iconBasePath.replace(/\/$/, '');
  const needsSvgInjection = (element) => (
    !element.hasAttribute('data-svg-injected') || !element.querySelector('svg')
  );
  const undecoratedIcons = [
    ...(root.matches?.('span.icon') && needsSvgInjection(root) ? [root] : []),
    ...[...root.querySelectorAll('span.icon')].filter(needsSvgInjection),
  ];
  const icons = undecoratedIcons
    .map((element) => {
      const authoredName = [...element.classList].find((className) => (
        className !== 'icon-milo' && /^icon-[a-z0-9][a-z0-9_-]*$/i.test(className)
      ))?.slice(5);
      return {
        element,
        name: iconAliases[authoredName] || authoredName,
      };
    })
    .filter(({ name }) => name);
  ...
  await Promise.all(icons.map(async ({ element, name }) => {
    ...
    if (existingSvg) {
      element.dataset.svgInjected = 'true';
      return;
    }
    const iconUrl = `${basePath}/${name}.svg`;
    try {
      const svgText = await loadInlineIconSvg(iconUrl);
      const svg = createInlineSvg(svgText, element.ownerDocument);
      if (!svg) throw new Error(`Invalid SVG returned for icon: ${name}`);
      element.replaceChildren(svg);
      element.dataset.svgInjected = 'true';
    } catch (error) {
      window.lana?.log(`Failed to load authored icon :${name}:: ${error.message}`);
    }
  }));
}
