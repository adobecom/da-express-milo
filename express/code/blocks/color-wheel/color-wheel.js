    ...
    try {
      ...
      layoutInstance = await createColorToolLayout(block, {
        ...
        actionMenu: {
          ...
          transformPalette: (rawHexes) => {
            if (activeHarmonyRule !== 'CUSTOM') return rawHexes;
            const { lockedByIndex } = swatchRailController.getState();
            if (!lockedByIndex?.size) return rawHexes;
            const currentHexes = controller.getState().swatches.map((s) => s.hex);
      ...
      const onHistoryChange = () => {
        ...
        if (isRandom && activeHarmonyRule !== 'CUSTOM') {
          controller.replaceSwatchesFromHexes(palette, { baseIndex: 0, harmonyRule: activeHarmonyRule });
