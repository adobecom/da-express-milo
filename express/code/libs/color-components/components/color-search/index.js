...
class ColorSearch extends LitElement {
    ...
    fetchPalettes = (event: KeyboardEvent) => {
        ...
        */
        if(event.key === 'Enter' && query !== this.searchQuery && !this.isFetching) {

            // Do not send a search request if User have not typed anything. Just clear the existing search results.
            if (this.query === '') {
    ...
    render() {
        return html`
            <div class="color-search">
                <button class='icon-search'><label for='palette-search' aria-label=${this.searchPlaceholder}><x-icon-search class='icon-search-logo'></x-icon-search></label></button>
                <input type='text' id='palette-search' data-testid='search-palettes-input' @keyup=${this.fetchPalettes} placeholder=${this.searchPlaceholder}></input>
                ${this.query ? html`<button class='icon-close' @click=${this.resetResults}>
                    <span></span>
                    <span></span>
                </button>` : null}
            </div>
