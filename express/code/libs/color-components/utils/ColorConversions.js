...
export const validateHEX = (hex) => {
  ...
  const newHex = hex.replace(shorthandRegex, (m, r, g, b, a) => {
    ...
    return fullHex;
            ...
            const result = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})?$/i.exec(newHex); // eslint-disable-line
...
export const removeTransparencyFromHEX = (hex) => {
...
export const hexToHue = (hex) => hexToHSL(hex)?.hue ?? 0;
export const hexToAlpha = (hex) => {
  const result = validateHEX(hex);

  return result && result[4] ? toFixedNumber(parseInt(result[4], 16) / 255, 2) : 1;
};
export const hexAlphaToHexa = (hex, alpha) =>
// If hex already has alpha, remove it first
  removeTransparencyFromHEX(hex) + toHEX(Math.round(alpha * 255));
export const hslToHEX = (hue, saturation, lightness) => {
  const rgb = hslToRGB(hue, saturation, lightness);

  return rgbToHex(rgb);
};
export const hslaToHEXA = (hue, saturation, lightness, alpha) => {
  const rgb = hslToRGB(hue, saturation, lightness);
  const hex = rgbToHex(rgb) + toHEX(toFixedNumber(alpha * 255, 0));

  return hex;
};
export const hsbToHEX = (hue, saturation, brightness) => {
  const rgb = hsbToRGB(hue, saturation, brightness);

  return rgbToHex(rgb);
};
export const hsbaToHEXA = (hue, saturation, brightness, alpha) => {
  const rgb = hsbToRGB(hue, saturation, brightness);
  const hex = rgbToHex(rgb) + toHEX(toFixedNumber(alpha * 255, 0));

  return hex;
};
export const rgbStringToHex = (colorString) => {
...
export const rgbToHex = ({ red, green, blue }) => `#${toHEX(red)}${toHEX(green)}${toHEX(blue)}`;
export const toHEX = (number) => {
  const hex = number.toString(16).toUpperCase();

  return (hex.length === 1 ? '0' : '') + hex;
};
export const rgbToHSB = (red, green, blue) => {
  let hue = 0;

  // Faster version of Math.min(r, g, b);
  /* eslint-disable no-extra-parens */
  const min = (red < green && red < blue) ? red : (green < blue) ? green : blue;
  const brightness = (red > green && red > blue) ? red : (green > blue) ? green : blue;
  // Faster version of Math.max(r, g, b)
  const saturation = (brightness === 0) ? 0 : (brightness - min) / brightness;
  const delta = (saturation === 0) ? 0.00001 : brightness - min;
...
export const hsbToRGB = (hue, saturation, brightness) => {
  let red; let green; let blue; let i; let f; let p; let q; let
    t;
...
/**
    ...
    */
export const hslToHSB = (hue, saturation, lightness) => {
  saturation /= 100;
  lightness /= 100;
...
/**
    ...
    */
export const hsbToHSL = (hue, saturation, brightness) => {
  saturation /= 100;
  brightness /= 100;

  const lightness = brightness * (1 - saturation / 2);
  const hslSaturation = lightness === 0 || lightness === 1 ? 0 : ((brightness - lightness) / Math.min(lightness, 1 - lightness));
...
export const polarToXy = (r, phi) => {
  const x = r * Math.cos(phi);
  const y = r * Math.sin(phi);

  return [x, y];
};
export const degToRad = (deg) => deg * Math.PI / 180 - Math.PI;
