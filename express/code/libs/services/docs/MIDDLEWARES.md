...
- `auth.middleware.js` — enforces authenticated access. Checks `adobeIMS.isSignedInUser()` and throws `AuthenticationError` if the user is not logged in. Can be applied to all topics (string form) or scoped to specific topics using the conditional middleware config (object form). Dispatches the `service:ims:ready` event once the IMS SDK is available (see below).
