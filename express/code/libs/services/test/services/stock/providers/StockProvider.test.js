...
import StockProvider, { createStockProvider } from '../../../../providers/StockProvider.js';
import StockPlugin from '../../../../plugins/stock/StockPlugin.js';
import { CURATED_GALLERIES_STOCK } from '../../../../plugins/stock/constants.js';
...
describe('StockProvider', () => {
  ...
  beforeEach(() => {
    ...
    // Stub window.fetch for all HTTP-based actions
  ...
  describe('delegation wiring', () => {
    ...
    it('getFileRedirectUrl should return a URL string', () => {
      const result = provider.getFileRedirectUrl(123456);
      expect(result).to.be.a('string');
      expect(result).to.include('/images/id/123456');
    });

    it('getContributorUrl should return a URL string', () => {
      const result = provider.getContributorUrl(42);
      expect(result).to.be.a('string');
  ...
  describe('error boundary (safeExecute)', () => {
    ...
    it('getFileRedirectUrl should return null for invalid input', () => {
      const result = provider.getFileRedirectUrl(undefined);
      expect(result).to.be.null;
    });

    it('getContributorUrl should return null for invalid input', () => {
      const result = provider.getContributorUrl(null);
      expect(result).to.be.null;
    });
  });
