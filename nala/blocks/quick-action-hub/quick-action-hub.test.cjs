...
const { features } = require('./quick-action-hub.spec.cjs');
const QuickActionHubBlock = require('./quick-action-hub.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('QuickActionHubBlock Test Suite', () => {
  // Test Id : 0 : @quick-action-hub-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new QuickActionHubBlock(page, features[0].selector);
