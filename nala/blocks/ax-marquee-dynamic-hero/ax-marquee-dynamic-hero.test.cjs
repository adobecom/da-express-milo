...
const { features } = require('./ax-marquee-dynamic-hero.spec.cjs');
const AxMarqueeDynamicHeroBlock = require('./ax-marquee-dynamic-hero.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

const miloLibs = process.env.MILO_LIBS || '';

test.describe('AxMarqueeDynamicHeroBlock Test Suite', () => {
  // Test Id : 0 : @ax-marquee-dynamic-hero-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}${miloLibs}`;
    const block = new AxMarqueeDynamicHeroBlock(page, features[0].selector);
