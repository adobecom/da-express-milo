...
const { features } = require('./login-page.spec.cjs');
const LoginPageBlock = require('./login-page.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('LoginPageBlock Test Suite', () => {
  // Test Id : 0 : @login-page-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new LoginPageBlock(page, features[0].selector);
    console.info(`[Test Page]: ${testUrl}`);

    // Responsive backgrounds: only one picture img is visible per viewport (l at 1280px)
