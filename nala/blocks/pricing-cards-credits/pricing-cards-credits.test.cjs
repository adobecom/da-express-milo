...
const { features } = require('./pricing-cards-credits.spec.cjs');
const PricingCardsCreditsBlock = require('./pricing-cards-credits.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('PricingCardsCreditsBlock Test Suite', () => {
  // Test Id : 0 : @pricing-cards-credits-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new PricingCardsCreditsBlock(page, features[0].selector);
