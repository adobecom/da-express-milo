...
const { features } = require('./embed.spec.cjs');
const EmbedBlock = require('./embed.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

const miloLibs = process.env.MILO_LIBS || '';

test.describe('EmbedBlock Test Suite', () => {
  // Test Id : 0 : @block-embed-embed-
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}${miloLibs}`;
    const embed = new EmbedBlock(page, features[0].selector);
    ...
    await test.step('step-2: Verify block content', async () => {
      await expect(embed.block).toBeVisible();
      const sem = data.semantic;

      for (const t of sem.texts) {
        const locator = embed.block.locator(t.selector).nth(t.nth || 0);
        await expect(locator).toContainText(t.text);
      }

      for (const m of sem.media) {
        const locator = embed.block.locator(m.selector).nth(m.nth || 0);
      ...
      for (const iEl of sem.interactives) {
        const locator = embed.block.locator(iEl.selector).nth(iEl.nth || 0);
    ...
    // MWPW-184252 - Skipping accessibility test for EmbedBlock until fixed.
    await test.step('step-3: Accessibility validation', async () => {
      await runAccessibilityTest({ page, testScope: embed.block, skipA11yTest: true });
    });
