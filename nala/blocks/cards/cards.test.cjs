...
const { features } = require('./cards.spec.cjs');
const CardsBlock = require('./cards.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

const miloLibs = process.env.MILO_LIBS || '';

test.describe('CardsBlock Test Suite', () => {
  // Test Id : 0 : @cards-dark
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}${miloLibs}`;
    const block = new CardsBlock(page, features[0].selector);
  ...
  // Test Id : 1 : @cards-default
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}${miloLibs}`;
    const block = new CardsBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
