...
const { features } = require('./browse-by-category.spec.cjs');
const BrowseByCategoryBlock = require('./browse-by-category.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

const miloLibs = process.env.MILO_LIBS || '';

test.describe('BrowseByCategoryBlock Test Suite', () => {
  // Test Id : 0 : @browse-by-category-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}${miloLibs}`;
    const block = new BrowseByCategoryBlock(page, features[0].selector);
  ...
  // Test Id : 1 : @browse-by-category-card
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}${miloLibs}`;
    const block = new BrowseByCategoryBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
  ...
  // Test Id : 2 : @browse-by-category-fullwidth
  test(`[Test Id - ${features[2].tcid}] ${features[2].name} ${features[2].tags}`, async ({ page, baseURL }) => {
    const { data } = features[2];
    const testUrl = `${baseURL}${features[2].path}${miloLibs}`;
    const block = new BrowseByCategoryBlock(page, features[2].selector);
    console.info(`[Test Page]: ${testUrl}`);
