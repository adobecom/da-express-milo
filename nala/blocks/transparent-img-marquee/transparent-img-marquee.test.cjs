...
const TransparentImgMarqueeBlock = require('./transparent-img-marquee.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

const miloLibs = process.env.MILO_LIBS || '';

test.describe('TransparentImgMarqueeBlock Test Suite', () => {
  // Test Id : 0 : @transparent-img-marquee-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const testUrl = `${baseURL}${features[0].path}${miloLibs}`;
    const block = new TransparentImgMarqueeBlock(page, features[0].selector);
  ...
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    ...
    const block = new TransparentImgMarqueeBlock(page, features[1].selector);
  ...
  test(`[Test Id - ${features[2].tcid}] ${features[2].name} ${features[2].tags}`, async ({ page, baseURL }) => {
    ...
    const block = new TransparentImgMarqueeBlock(page, features[2].selector);
