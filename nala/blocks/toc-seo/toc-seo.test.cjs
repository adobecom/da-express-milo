...
test.describe('TocSeoBlock Test Suite', () => {
  ...
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    ...
    await test.step('step-3: Verify position on desktop', async () => {
      const tocTopPosition = await block.block.evaluate((el) => getComputedStyle(el).getPropertyValue('--toc-top-position').trim());
      const numericValue = parseFloat(tocTopPosition);
  ...
  // Test Id : 1 : @toc-seo-blog
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}`;
    const block = new TocSeoBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
