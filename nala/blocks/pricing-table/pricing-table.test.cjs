...
const { features } = require('./pricing-table.spec.cjs');
const PricingTableBlock = require('./pricing-table.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('PricingTableBlock Test Suite', () => {
  // Test Id : 0 : @pricing-table-sticky-show3-sticky-show3-few-cols
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new PricingTableBlock(page, features[0].selector);
  ...
  // Test Id : 1 : @pricing-table-sticky-show3-sticky-show3
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}`;
    const block = new PricingTableBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
  ...
  // Test Id : 2 : @pricing-table-sticky-show3-sticky-show3-many-cols
  test(`[Test Id - ${features[2].tcid}] ${features[2].name} ${features[2].tags}`, async ({ page, baseURL }) => {
    const { data } = features[2];
    const testUrl = `${baseURL}${features[2].path}`;
    const block = new PricingTableBlock(page, features[2].selector);
    console.info(`[Test Page]: ${testUrl}`);
