...
const { features } = require('./make-a-project.spec.cjs');
const MakeAProjectBlock = require('./make-a-project.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('MakeAProjectBlock Test Suite', () => {
  // Test Id : 0 : @make-a-project-default
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new MakeAProjectBlock(page, features[0].selector);
  ...
  // Test Id : 1 : @make-a-project-centered-dark
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}`;
    const block = new MakeAProjectBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
