...
const { features } = require('./text.spec.cjs');
const TextBlock = require('./text.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('TextBlock Test Suite', () => {
  // Test Id : 0 : @text-max-width-8-desktop-center-xxl-spacing-xl-button-l-title-text-block-con-block
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new TextBlock(page, features[0].selector);
  ...
  // Test Id : 1 : @text-center-xxs-spacing-text-block-con-block
  test(`[Test Id - ${features[1].tcid}] ${features[1].name} ${features[1].tags}`, async ({ page, baseURL }) => {
    const { data } = features[1];
    const testUrl = `${baseURL}${features[1].path}`;
    const block = new TextBlock(page, features[1].selector);
    console.info(`[Test Page]: ${testUrl}`);
  ...
  // Test Id : 2 : @text-no-spacing-bottom-text-block-con-block
  test(`[Test Id - ${features[2].tcid}] ${features[2].name} ${features[2].tags}`, async ({ page, baseURL }) => {
    const { data } = features[2];
    const testUrl = `${baseURL}${features[2].path}`;
    const block = new TextBlock(page, features[2].selector);
    console.info(`[Test Page]: ${testUrl}`);
  ...
  // Test Id : 3 : @text-l-lockup-max-width-8-desktop-center-xxl-spacing-xl-button-l-title-text-block-con-block
  test(`[Test Id - ${features[3].tcid}] ${features[3].name} ${features[3].tags}`, async ({ page, baseURL }) => {
    const { data } = features[3];
    const testUrl = `${baseURL}${features[3].path}`;
    const block = new TextBlock(page, features[3].selector);
    console.info(`[Test Page]: ${testUrl}`);
