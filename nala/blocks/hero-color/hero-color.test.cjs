...
const { features } = require('./hero-color.spec.cjs');
const HeroColorBlock = require('./hero-color.page.cjs');
const { runAccessibilityTest } = require('../../libs/accessibility.cjs');
const { runSeoChecks } = require('../../libs/seo-check.cjs');

test.describe('HeroColorBlock Test Suite', () => {
  // Test Id : 0 : @hero-color-light
  test(`[Test Id - ${features[0].tcid}] ${features[0].name} ${features[0].tags}`, async ({ page, baseURL }) => {
    const { data } = features[0];
    const testUrl = `${baseURL}${features[0].path}`;
    const block = new HeroColorBlock(page, features[0].selector);
