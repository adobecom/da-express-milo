...
describe('font-generator/fontCard', () => {
  ...
  describe('copy interaction', () => {
    ...
    it('builds a hover "copy" prompt and a success zone inside the overlay', () => {
      const card = createFontCard(FONT, 'Hello', 16);
      const overlay = card.querySelector('.font-card-copy-overlay');
      expect(overlay).to.exist;
      expect(overlay.querySelector('.font-card-copy-prompt .font-card-copy-prompt-label').textContent)
    ...
    it('copies when the overlay (whole preview zone) is clicked, not just the button', async () => {
      const card = createFontCard(FONT, 'Hello', 16);
      card.querySelector('.font-card-copy-overlay').click();
      await writeTextStub.returnValues[writeTextStub.returnValues.length - 1].catch(() => {});
      await Promise.resolve();
      expect(writeTextStub.calledOnce).to.be.true;
      expect(card.classList.contains('is-copied')).to.be.true;
