...
Orchestrates the `build-content-from-figma` and `build-block-from-figma` skills end-to-end: gather inputs once, author the content document, capture the resulting `.aem.live` URL, then build the Express block against that page.
...
|-------|----------|-------|
| Figma URL — Mobile (< 600 px) | At least one of the five | Frame link |
...
| Figma URL — Widescreen (≥ 1680 px) | | Frame link (optional) |
| DA organization | Yes | e.g. `adobecom` |
| DA repository | Yes | e.g. `da-express-milo` |
...
Once the content skill finishes, parse the live URL from its Phase 7d / Phase 9 summary:

```
https://main--<repo>--<org>.aem.live/<da-path>/<page-name>
```

If the live URL is not present (e.g. the user declined to publish, or publishing failed), stop and report the situation. Do not invent a URL.
...
- **Base branch**: the user's value, or `stage` if not provided.

The block skill will detect remote-branch-mode from the `.aem.live` URL and handle feature-branch creation, branch-prefixed URL testing, visual comparison, accessibility, and performance audits on its own.
