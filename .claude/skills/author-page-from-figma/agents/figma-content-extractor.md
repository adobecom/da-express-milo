...
Delegated from Phase 3 of the main SKILL.md. Extracts structured
...
- **Icons**: small graphic ≤ 48px, typically above or beside the eyebrow/heading.
  ...
  Phase 5 exports the composite SVG via `use_figma` + `exportAsync`.
...
> **Record the full instance-specific node ID**, including the
> semicolon-chained instance form when `data-node-id` shows one (e.g.
> `I3941:57367;3931:19024`), not just the trailing plain ID. Phase 5
> needs this exact form to resolve the real per-instance asset via
> `get_design_context` — a plain ID alone may only resolve to the
> master component's default fill.

> **Do not download or base64-encode images.** Only capture Figma asset
> URLs and node IDs. Phase 5 handles all media transfers.
