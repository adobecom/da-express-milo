...
import type { PageStatus, PublishResult, HlxStatusResponse } from '../da/types.js';
import { getRequestToken } from '../lib/auth.js';

export const publish = new Hono();

function resolveStatus(s: HlxStatusResponse): PageStatus {
  if (s.live?.status === 200) return 'Published';
  if (s.preview?.status === 200) return 'Previewed';
  return 'Draft';
}
...
publish.get('/status', async (c) => {
  ...
  const raw = await da.getPageStatus(path, token);
...
publish.post('/', zValidator('json', singlePathSchema), async (c) => {
  ...
  const token = getRequestToken(c);
  const liveUrl = await da.publishPage(path, token);
...
publish.delete('/', zValidator('json', singlePathSchema), async (c) => {
  const { path } = c.req.valid('json');
  const token = getRequestToken(c);
  await da.unpublishPage(path, token);
...
publish.post('/bulk', zValidator('json', bulkSchema), async (c) => {
  ...
  const token = getRequestToken(c);
  const settled = await Promise.allSettled(paths.map((p) => da.publishPage(p, token)));
  const results: PublishResult[] = paths.map((path, i) => {
    const r = settled[i]!;
    return r.status === 'fulfilled'
      ? { path, success: true, url: r.value }
      : { path, success: false, error: (r as PromiseRejectedResult).reason?.message };
  });
...
publish.delete('/bulk', zValidator('json', bulkSchema), async (c) => {
  const { paths } = c.req.valid('json');
  const token = getRequestToken(c);
  const settled = await Promise.allSettled(paths.map((p) => da.unpublishPage(p, token)));
