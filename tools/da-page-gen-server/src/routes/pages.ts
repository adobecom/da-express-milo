...
import type { DAASPage, PageStatus } from '../da/types.js';
...
pages.get('/', async (c) => {
  ...
  const settled = await Promise.allSettled(
    htmlFiles.map(async (f): Promise<DAASPage | null> => {
      const html = await da.getSource(f.path);
      const templatePath = getTemplatePath(html);
      if (!templatePath) return null; // not a DAAS-generated page
      const fields = extractDAASFields(html);
      const webPath = stripHtml(toWebPath(f.path));
      return { path: webPath, templatePath, fields, status: 'Draft' as PageStatus };
    }),
  );

  const result = settled
    .filter((r): r is PromiseFulfilledResult<DAASPage | null> => r.status === 'fulfilled')
    .map((r) => r.value)
    .filter((p): p is DAASPage => p !== null);
...
pages.get('/file', async (c) => {
  ...
  if (!webPath) return c.json({ error: 'Missing ?path query parameter' }, 400);
...
pages.post('/fields/bulk', zValidator('json', bulkFieldSchema), async (c) => {
  ...
  return c.json({
    ...
    errors: settled
      .map((r, i) =>
        r.status === 'rejected'
          ? { path: webPaths[i], error: (r as PromiseRejectedResult).reason?.message }
          : null,
      )
      .filter(Boolean),
  });
