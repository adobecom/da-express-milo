 ...
 *
 * POST /generate/batch
 *   Bulk page generation from pre-resolved fields.
 *
 * POST /generate/from-bindings
 *   Full pipeline: raw records + binding definition → generated pages.
 */
...
import { toSourcePath, ensureHtml, toEditUrl } from '../lib/paths.js';
import { resolveFields, resolveDestPattern, type Binding } from '../lib/bindings.js';
...
generate.post('/', zValidator('json', generateSchema), async (c) => {
  ...
  const templateHtml = await da.getSource(ensureHtml(templateSourcePath), token);
  const populated = fillTemplate(templateHtml, fields);

  const sourceDest = toSourcePath(destPath);
  const result = await da.postSource(sourceDest, populated, 'text/html', token);

  return c.json(
    {
      destPath,
      editUrl: result.source?.editUrl ?? toEditUrl(sourceDest),
...
generate.post('/pdp', zValidator('json', pdpSchema), async (c) => {
  ...
  const templateHtml = await da.getSource(ensureHtml(tplPath), token);
...
generate.post('/batch', zValidator('json', batchSchema), async (c) => {
  ...
  const settled = await Promise.allSettled(
    items.map(async ({ destPath, fields }) => {
      const populated = fillTemplate(templateHtml, fields);
      const sourceDest = toSourcePath(destPath);
      const result = await da.postSource(sourceDest, populated, 'text/html', token);
      return {
  ...
  const results = settled.map((r, i) =>
    r.status === 'fulfilled'
      ...
      : { destPath: items[i]!.destPath, success: false, error: (r as PromiseRejectedResult).reason?.message },
  );
...
generate.post('/from-bindings', zValidator('json', fromBindingsSchema), async (c) => {
  ...
  const settled = await Promise.allSettled(
    records.map(async (record) => {
      ...
      const fields = resolveFields(bindings as Binding[], record);
      const populated = fillTemplate(templateHtml, fields);
      const sourceDest = toSourcePath(destPath);
      const result = await da.postSource(sourceDest, populated, 'text/html', token);
      return { destPath, editUrl: result.source?.editUrl ?? toEditUrl(sourceDest), liveUrl: result.aem?.liveUrl };
    }),
  );
